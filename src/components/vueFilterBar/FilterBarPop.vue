    ...
    <div class="filterbarpop">
      ...
      <div class="main">
        <div class="main-sidebar" :class="{'full-line': !items,'bg-style':items,'line-style':!items,}">
          <div v-if="menu.type !== 'filter'" class="item" @click="clickSidebar(sidemenu, index)" v-for="(sidemenu, index) in sideMenus.detailList" :class="{'selected': currentSelectIndex == index}">
          ...
          <div v-if="menu.type == 'filter'" v-for="(sm, _index) in menu.tabs">
            ...
            <div v-if="sm.type == 'radio'" class="filter-item">
              <span v-for="(sidemenu, index) in sm.detailList" class="item-operation" @click="clickRadioFilterbar(sm, _index, index)" :class="{'multi-selected': sidemenu.isActive == true}">
                  {{ sidemenu.name }}
                </span>
            </div>
            <div v-else-if="sm.type == 'checkbox'" class="filter-item">
              <span v-for="(sidemenu, index) in sm.detailList" class="item-operation" @click="clickCheckboxFilterbar(sm, _index, index)" :class="{'multi-selected': sidemenu.selectIndex == index}">
                {{ sidemenu.name }}
                </span>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    // 筛选方法
    clickRadioFilterbar(v, I, i){
       v.detailList.forEach(function (obj) {
        obj.isActive = false;
      });
      console.log("点击", v.detailList);
      v.detailList[i].isActive = !v.detailList[i].isActive;
      console.log("点击", v.detailList[i]);
       if (!this.range[I]) {
        this.range[I] = {
          name: v.name,
          value: {}
