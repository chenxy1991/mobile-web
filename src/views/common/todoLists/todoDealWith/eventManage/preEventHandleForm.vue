<template>
<div class="preEventHandleForm">
   <div  class="reset-cell" style="font-size: 1.4rem;background-color:rgb(242,243,238);">
    <header class="head">
   ...
   <main>
    ...
    <!--   <div id="appendTbody">
        <satisfaction-degree   v-if="showAppendTbody==true"
          ...
          :currentTaskInfoId="taskInfoId"
          :processDefinitionKey="processDefinitionKey"
          :currentTaskName="currentTaskName"
           contentTemplateId="contentTemplateId_satisfaction"
    ...
    <popup v-if="showDepartmentManagement" v-model="showDepartmentManagement" @on-hide="hideAddOrgOrUser('hide')" @on-show="showAddOrgOrUser('show')" position="right" width="100%">
      <send-info-dialog v-on:hideDepartmentManagementDialog="closeDepartmentManagementDialog" :parms="dprMarEntryParameter" :decisionTransition="decisionTransition" :OrgList="openDialogOrgList" v-on:sureDprMgrBtn="sureDprMgrBtn"></send-info-dialog>
    </popup>
    <!--显示常用小组-->
    <popup v-if="showComGroupTree" v-model="showComGroupTree" @on-hide="hideComGroupTree('hide')" @on-show="showComGroupTree('show')" position="right" width="100%">
      <common-group-tree v-on:hideComGroupTreeDialog="closeComGroupTreeDialog" :activityId="activityId" :processDefinitionKey="processDefinitionKey" :decisionTransition="decisionTransition" v-on:sureComGroupTreeBtn="sureComGroupTreeBtn"></common-group-tree>
    </popup>
 ...
 data() {
  ...
  created: function() {
   const {
    ...
    this.dealEventForm.businessType=formData.businessType;
    this.dealEventForm.processDefinitionKey = processKey;
    this.dealEventForm.businessKey = businessKey;
    this.dealEventForm.taskInfoId = taskInfoId;
    this.dealEventForm.currentTaskName = currentTaskName;
    this.dealEventForm.applyOrgId=applyOrgId;
  ...
  methods: {
    ...mapActions([
      ...
      handleWorkOrderTaskClick(){
       ...
       console.log("formData111111111111111111",formDataStr);
       if (!isEmpty(this.dealEventForm)) {
           ...
           console.log("this.selectedSendList2222222222222222",this.selectedSendList);
           let selectRoutebtnValue=JSON.stringify(this.selectRoutebtnValue);
            ...
            query: {
              ...
              selectRoutebtnValue:selectRoutebtnValue,
    ...
    async initPageData() {
       ...
       */
      let formButtonsJson = JSON.parse(formButtons);
      let humanTaskJson = JSON.parse(humanTaskObj);
      let name = formButtonsJson.name;
      let beforeOnclick = formButtonsJson.beforeOnclick;
      let onclick = formButtonsJson.onclick;
      let initDialogSuccess = formButtonsJson.initDialogSuccess;
      let defaultFirstSelected = formButtonsJson.defaultFirstSelected;
      let routeButtons = formButtonsJson.routeButtons; //是	JSON	弹出窗口路由
      this.dealEventForm.currentDealOrgId=humanTaskJson.dealOrgId;
      if (!isEmpty(beforeOnclick)) {
        if (beforeOnclick === "f_beforeOpenDialog") {
          this.f_beforeOpenDialog(humanTaskJson, formButtonsJson);
        } else if (beforeOnclick === "f_beforeOpenDialog_exit") {
          this.f_beforeOpenDialog_exit(humanTaskJson, formButtonsJson);
        }
      } else if (isEmpty(onclick)) {
        //alert("点击操作按钮触发事件，为空时，默认触发打开选人窗口");
        this.getOpenRouteButtons("", formButtonsJson);
      }
      //路由规则
      console.log("任务分配前置规则 (下一环节)-派往", formButtonsJson);
    }, //初始化页面数据
     async f_beforeOpenDialog(humanTaskJson, formButtonsJson) {
      let hiddenRouteButton = "";
      if (humanTaskJson === undefined) {
        alert("humanTaskJson===undefined");
        return;
      }
      let result = {};
      ...
      const sendInfo = {
        ...
        Jsondata: {
          operType: "屏蔽路由",
          humanTaskJson: humanTaskJson,
          simulationStatus: "",
          applyOrgId: String(this.$route.query.applyOrgId)
     ...
     this.$vux.loading.show({ text: '努力加载中'  });
      ...
      if (result.meta.success == true) {
        ...
        console.log("提交屏蔽路由规则data",data.showPreDealRoute);
         	if(data.showPreDealRoute==='1'){
                hiddenRouteButton="送申告人确认"
              //$("#hiddenRouteButton").val("送申告人确认");
            }else if(data.showReportRoute3==='1'){
                  hiddenRouteButton="提交分派,提交复核"
            }else{
              hiddenRouteButton="提交复核";
                  // hiddenRouteButton="提交复核"
            }
        console.log("隐藏的路由hiddenRouteButton", hiddenRouteButton);
        //第二
        this.getOpenRouteButtons(hiddenRouteButton, formButtonsJson);
      } else{
     ...
     async f_beforeOpenDialog_exit(humanTaskJson, formButtonsJson) {
      let hiddenRouteButton = "";
      if (humanTaskJson === undefined) {
        alert("humanTaskJson===undefined");
        return;
      ...
      if (result.meta.success == true) {
        let data = result.data.returnMap;
       if(data.showReportRoute2==="1"){
          hiddenRouteButton="退回分派";
        }else if(data.showPreDealRoute==="1"&&data.showReportRoute2==="0"){
         hiddenRouteButton="退回";
        }
        console.log("隐藏的路由hiddenRouteButton", hiddenRouteButton);
          //第二
         this.getOpenRouteButtons(hiddenRouteButton, formButtonsJson);
     ...
     async getOpenRouteButtons(hiddenRouteButton, formButtonsJson) {
    ...
    routerBtnChange(val, label) {
      ...
      if (!isEmpty(key)) {
        ...
        console.log("理由列表信息改变的key", selectRoutebtnValue);
           /*
     ...
     async getDefualtOrgUsersTreeData(selectRoutebtnValue) {
      ...
      if (result.meta.success == true) {
        ...
        this.activityId = returnMap.activityId;
     ...
     try {
      ...
      if (result.meta.success == true) {
        ...
        for (let userInfo of defaultSendInfoList) {
           this.haveCheckerUserList.push(userInfo.userId);
        }
