<template>
<div class="eventApplyForm">
  <div class="reset-cell" style="font-size: 1.4rem;">
    <header class="head">
  ...
  methods: {
    ...
    async initPageData() {
      ...
      let routeButtons = formButtonsJson.routeButtons; //是	JSON	弹出窗口路由
       this.dealEventForm.currentDealOrgId=JSON.parse(humanTaskObj).dealOrgId;
    ...
    async f_beforeOpenDialog(humanTaskJson, formButtonsJson) {
     ...
     this.$vux.loading.show({ text: '努力加载中'  });
      ...
      if (result.meta.success == true) {
        ...
        let taskInfoId=this.taskInfoId;
        if(data.showPreDealRoute=="1"){
          hiddenRouteButton="提交分派";
          if(data.applyOrgId!=28838&&taskInfoId==""){
            hiddenRouteButton="提交分派,直接归档";
          }

        }else{
          hiddenRouteButton="提交一线处理";
          if(data.applyOrgId!=28838&&taskInfoId==""){
            hiddenRouteButton="提交一线处理,直接归档";
          }
        }
