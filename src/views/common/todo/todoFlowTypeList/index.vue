<template>
<div class="todoFlowTypeList" id="todoFlowTypeList" >
  ...
  <mescroll-vue id="mescroll" ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
      <article>
      <group :gutter="0">
      <cell  :border-intent="false" :title="item.processName" is-link @click.native="handleTaskFlowTypeClick(item, 'todoFlowList')" v-for="(item,idx) in todoTaskFlowTypeLists" :key="idx"   >
...
export default {
  ...
  created: function () {
    const todoType = this.$route.query.type;
  ...
  methods: {
    ...
    upCallback(page, mescroll) {
      this.runQueryTodoTaskFlowTypeListsApi(page,mescroll);
    },
    async runQueryTodoTaskFlowTypeListsApi(page,mescroll) {
      let result = {};
      ...
      try {
        ...
        console.error(error);
        this.alert('请求工单流程类型异常，原因:' +error);
      }
      ...
      if (result.meta.success == true) {
        ...
        this.alert('请求工单流程类型失败，原因:' +failDesc+","+ failCaused);
        //跳转到404页面
      }
    },
