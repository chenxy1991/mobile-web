  ...
  methods: {
    ...
    async getOpenRouteButtons(hiddenRouteButton) {
     ...
     routeButtons.beforeConfirmOfDialogYes 否 String 点击弹出窗口确定按钮时触发事件 返回值： true: 确认提交； false: 不能提交 */
          ...
          /* this.dealEventForm.nextActivityId=nextActivityId;
          this.dealEventForm.contentTemplateId=contentTemplateId;
          this.dealEventForm.decisionTransition=decisionTransition; */
           //加载模板
     ...
     async getDefualtOrgUsersTreeData(selectRoutebtnValue) {
      ...
      if (result.meta.success == true) {
        ...
        if (returnMap.defualtOrgsAndUsersList) {
          ...
          let defualtCountersignOrgsAndUsersList=returnMap.defualtCountersignOrgsAndUsersList;
          if (!isEmpty(defualtCountersignOrgsAndUsersList)) {
    ...
    async getToselectUsersTree(returnMap) {
     ...
     try {
      ...
      if (result.meta.success == true) {
        ...
        console.log("this.haveCheckerUserList",this.haveCheckerUserList);
