...
export default {
  ...
  methods: {
    ...
    async runSaveTaskInfoApi() {
      ...
      let userId=JSON.parse(this.storejs.get('LoginUserId'));
       try {
      const eventForm = Object.assign({}, this.preStepEventForm,{
      ...
      console.log("表单提交内容eventForm",eventForm);
      console.log("表单提交内容formDataMap",formDataMap);
      const sendInfo = {
        headers: {
          userId: userId,
        },
    ...
    validSaveTask() {
      //满意度信息校验
       ...
       let satisfactionTaskInfoId="";
       let satisfactionObj= this.satisfactionValue;
      let contentTemplateId=this.preStepEventForm.contentTemplateId;
        //初始化满意度评价
     ...
     let beforeConfirmOfDialogYes=this.preStepEventForm.beforeConfirmOfDialogYes;
